...
# IMDB lookup
try:
    from imdb_lookup import IMDBSeriesInfo, lookup_series
    IMDB_AVAILABLE = True
except ImportError:
...
try:
    ...
    from core.ai_metadata_extractor import MediaType as AIMediaType
    from core.ai_metadata_extractor import VideoMetadata as AIVideoMetadata
...
class MediaFile:
    ...
    def __init__(self, path: Path, original_name: str):
        ...
        self.cleaned_name: str | None = None
        self.format_detected: str | None = None
        self.is_series: bool = False
        self.series_name: str | None = None
        self.season: int | None = None
        self.episode: int | None = None
        self.year: int | str | None = None
        self.episode_title: str | None = None
        self.audio_tracks: list[dict] | None = None
        self.video_tracks: list[dict] | None = None
        self.subtitle_tracks: list[dict] | None = None
...
class SeriesDetector:
    ...
    @classmethod
    def get_series_info(cls, series_name: str) -> tuple[str, str | None]:
    ...
    @staticmethod
    def detect_series(filename: str, use_ai: bool = True) -> tuple[bool, str | None, int | None, int | None, str | None, str | None]:
        """
    ...
    @staticmethod
    def create_folder_name(series_name: str, year: str | None) -> str:
        """Create sanitized folder name."""
        name = f"{series_name} ({year})" if year else series_name
        return Patterns.INVALID_CHARS.sub('_', name).strip()

    @staticmethod
    def create_episode_filename(series_name: str, season: int, episode: int,
                                 year: str | None, extension: str,
                                 episode_title: str | None = None) -> str:
...
class MediaOrganizer:
    ...
    __slots__ = ('_media_extensions', 'cleaners', 'series_detector')
    ...
    def __init__(self):
        ...
        # Use set for O(1) extension lookup (Hack #1) - use centralized constant
        self._media_extensions: set[str] = set(VIDEO_EXTENSIONS)

    def clean_filename(self, filename: str) -> tuple[str, str | None]:
    ...
    def organize_files(self, directory: str | Path = '.', output_dir: str | Path | None = None) -> list[MediaFile]:
        ...
        series_groups: dict[tuple[str, str | None], list[MediaFile]] = {}
    ...
    def _organize_series(self, base_dir: Path, series_name: str, year: str | None, episodes: list[MediaFile]):
...
class AudioTrackFilter:
    ...
    def __init__(self):
        # Import centralized language keywords
        from core.language_utils import LANGUAGE_KEYWORDS
        self.language_keywords = LANGUAGE_KEYWORDS

    def get_track_info(self, file_path: Path) -> dict | None:
    ...
    def filter_audio(self, file_path: Path, target_language: str = 'malayalam',
                     output_path: Path | None = None, volume_boost: float = 1.0,
        ...
        elif track_info['subtitle_tracks']:
    ...
    def batch_filter(self, directory: str | Path, target_language: str = 'malayalam',
