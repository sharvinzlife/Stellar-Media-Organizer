...
class SeriesDetector:
    ...
    @staticmethod
    def detect_series(filename: str) -> tuple[bool, str | None, int | None, int | None, int | None]:
        """
    ...
    @staticmethod
    def create_series_folder_structure(base_path: Path, series_name: str, year: int | None = None) -> Path:
    ...
    @staticmethod
    def create_episode_filename(series_name: str, season: int, episode: int, year: int | None, extension: str) -> str:
        """Create properly formatted episode filename."""
        base_name = f"{series_name} ({year})" if year else series_name

        episode_name = f"{base_name} - S{season:02d}E{episode:02d}{extension}"
        return re.sub(r'[<>:"|*?]', "_", episode_name)
...
class MediaOrganizer:
    ...
    def organize_files(self, directory: str | Path) -> list[MediaFile]:
    ...
    def _organize_series_files(self, base_dir: Path, series_name: str, year: int | None, episodes: list[MediaFile]):
    ...
    def _process_directories(self, directory: Path, processed_files: list[MediaFile]):
        ...
        for dir_path in directory.iterdir():
            ...
            cleaned_name, _format_detected = self.clean_filename(dir_path.name)
...
class AudioTrackFilter:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.language_keywords = {k: list(v) for k, v in LANGUAGE_KEYWORDS.items()}
        except ImportError:
    ...
    def check_mkvtoolnix_available(self) -> bool:
        ...
        try:
            ...
            logger.exception("MKVToolNix not found")
            return False
    ...
    def get_track_info(self, file_path: Path) -> dict:
        ...
        try:
            ...
            logger.exception(f"Error getting track info for {file_path}: {e}")
            return {}
    ...
    def auto_select_language(self, file_path: Path) -> str | None:
        """
    ...
    def filter_language_audio(self, file_path: Path, target_language: str = "malayalam",
                             output_path: Path | None = None, volume_boost: float = 1.0) -> bool:
        ...
        try:
            ...
            logger.exception(f"Error filtering {file_path}: {e}")
            return False
    ...
    def _apply_volume_boost(self, file_path: Path, volume_boost: float) -> bool:
        ...
        try:
            ...
            logger.exception(f"Error applying volume boost: {e}")
            return False

    def batch_filter_directory(self, directory: str | Path, target_language: str = "malayalam",
...
class SmartNASRouter:
    ...
    def find_existing_series_folder(self, nas_base_path: Path, series_name: str, year: int | None = None) -> Path | None:
        """
    ...
    def get_series_destination(self, file_path: Path, nas_base_path: Path,
                                series_name: str, season: int, year: int | None = None) -> Path:
        """
    ...
    def process_file_to_nas(self, file_path: Path, nas_name: str, category: str,
        ...
        try:
            ...
            logger.exception(f"Error processing file to NAS: {e}")
