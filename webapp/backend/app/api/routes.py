...
from app.services.media_service import AudioTrackFilter, MediaOrganizer
...
try:
    ...
    USE_CENTRALIZED_CONSTANTS = True
except ImportError:
    USE_CENTRALIZED_CONSTANTS = False
    ...
    def get_nas_category_map(nas_name: str) -> dict:
        return {
            "malayalam movies": "malayalam movies",
            "movies": "movies",
            "bollywood movies": "bollywood movies",
...
async def analyze_files(request: AnalyzeRequest):
    ...
    try:
        ...
        logger.exception(f"Error analyzing files: {e}")
    ...
    try:
        ...
        if request.operation in ["organize", "both"]:
            try:
                ...
                errors.append(f"Organization error: {e!s}")
                logger.exception(f"Organization error: {e}")
        ...
        if request.operation in ["filter_audio", "both"]:
            try:
                ...
                errors.append(f"Audio filtering error: {e!s}")
                logger.exception(f"Audio filtering error: {e}")
...
async def convert_videos(request: VideoConversionRequest):
    ...
    try:
        ...
        logger.exception(f"Error converting videos: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def test_nas_connection(request: NASTestRequest):
    ...
    try:
        ...
        return {"success": False, "message": f"❌ Error testing connection: {e!s}"}
...
async def get_alldebrid_status():
    ...
    try:
        result = subprocess.run(["aria2c", "--version"], check=False, capture_output=True, timeout=5)
...
def run_alldebrid_download_task(job_id: int, request: AllDebridDownloadRequest):
    ...
    try:
        ...
        output_path = str(get_download_base_dir() / str(job_id))
        ...
        if free_gb < MIN_DISK_SPACE_GB:
            add_log(f"⚠️ Low disk space: {free_gb:.1f}GB free", "warning")
        else:
            add_log(f"💾 Disk space: {free_gb:.1f}GB available", "info")
        ...
        if request.download_only:
            ...
            for f in results.get("renamed", []):
                try:
                    ...
                    add_log(f"⚠️ Error tracking file: {e!s}", "warning")
                ...
                if nas_result:
                    ...
                    if settings.plex_enabled and settings.plex_auto_scan:
                        ...
                        try:
                            ...
                            add_log(f"⚠️ Plex scan failed: {plex_error!s}", "warning")
                else:
                    add_log(f"⚠️ NAS transfer failed, files remain in {output_path}", "warning")
            except Exception as nas_error:
                add_log(f"⚠️ NAS transfer error: {nas_error!s}", "warning")
        ...
        add_log(f"❌ Import error: {e!s} - alldebrid_downloader module not found", "error")
        job["status"] = "failed"
        job["error"] = str(e)
        job["completed_at"] = datetime.now().isoformat()
        job["duration"] = time.time() - start_time
        logger.exception(f"AllDebrid import error: {e}")
        save_jobs_to_file()  # Persist job history
    except Exception as e:
        add_log(f"❌ Error: {e!s}", "error")
        job["status"] = "failed"
        job["error"] = str(e)
        job["completed_at"] = datetime.now().isoformat()
        job["duration"] = time.time() - start_time
        logger.exception(f"AllDebrid download error: {e}")
        save_jobs_to_file()  # Persist job history


def transfer_to_nas(source_dir: str, nas_name: str, category: str, log_func, job_id: int | None = None) -> bool:
    ...
    # Use centralized category mapping (fallback already defined in imports)
...
def detect_content_type(filename: str, default_category: str, log_func, job_id: int | None = None) -> str:
    """
    ...
    # TV Show detection
    if use_centralized:
        is_tv_show = is_tv_content(filename)
    else:
        import re
        tv_patterns = [
            r"s\d{1,2}e\d{1,2}",
    ...
    if use_centralized:
        ...
        # Fallback keywords
        malayalam_keywords = ["malayalam", " mal ", "-mal-", ".mal.", "mlm", "[mal]", "(mal)"]
        hindi_keywords = ["hindi", "bollywood", " hin ", "-hin-", ".hin.", "[hin]", "(hin)"]
        
        if any(kw in filename_lower for kw in malayalam_keywords):
            detected_lang = "malayalam"
        elif any(kw in filename_lower for kw in hindi_keywords):
            detected_lang = "hindi"

    # If no language detected from filename, lookup from OMDB/TMDB
    if not detected_lang:
        metadata_lang = lookup_language_from_metadata(filename, is_tv_show, log_func)
        if metadata_lang:
            detected_lang = metadata_lang

    # Determine final category
    if use_centralized:
        detected = get_category_for_language(detected_lang, is_tv_show)
    else:
        if is_tv_show:
    ...
    if detected_lang:
        log_func(f"🎯 Detected language: {detected_lang.title()}", "info")
    else:
        log_func("🎯 Language: English/General", "info")
...
def lookup_language_from_metadata(filename: str, is_tv_show: bool, log_func) -> str:
            ...
            if resp.status_code == 200:
                ...
                if data.get("Response") == "True":
                    ...
                    if "india" in country and "english" not in language:
                        ...
                        log_func("✅ OMDB: Indian production, assuming Hindi", "info")
                        return "hindi"
        except Exception as e:
            log_func(f"⚠️ OMDB lookup failed: {e!s}", "warning")
    ...
    if settings.tmdb_api_key or settings.tmdb_access_token:
        try:
            ...
            log_func(f"⚠️ TMDB lookup failed: {e!s}", "warning")

    log_func("ℹ️ No Indian language detected, using English/General", "info")
    return None


# ========== Plex Integration ==========

# PLEX_LIBRARY_MAP is imported from core.constants (with fallback defined above)
...
def trigger_plex_scan(category: str, log_func=None) -> bool:
    ...
    try:
        ...
        logger.exception(f"Plex scan error: {e}")
        if log_func:
            log_func(f"❌ Plex scan error: {e!s}", "error")
        return False
...
async def get_job_logs(job_id: int):
    ...
    job = alldebrid_jobs[job_id]
    logs = []
    for _i, log in enumerate(job.get("logs", [])):
        logs.append({
...
async def get_plex_status():
    ...
    try:
        ...
        logger.exception(f"Plex status error: {e}")
        return {
            "success": False,
            "enabled": True,
            "configured": True,
...
async def get_plex_recently_added(library_key: str | None = None, limit: int = 20):
    ...
    try:
        ...
        logger.exception(f"Failed to get recently added: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/plex/match/{rating_key}")
async def match_plex_item(rating_key: str, imdb_id: str | None = None, title: str | None = None, year: str | None = None):
    ...
    try:
        ...
        logger.exception(f"Plex match error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def refresh_plex_item(rating_key: str):
    ...
    try:
        ...
        logger.exception(f"Plex refresh error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_tautulli_status():
    ...
    try:
        ...
        logger.exception(f"Tautulli status error: {e}")
        return {
            "success": False,
            "enabled": True,
            "configured": True,
...
async def get_tautulli_libraries():
    ...
    try:
        ...
        logger.exception(f"Failed to get Tautulli libraries: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_tautulli_popular(media_type: str = "movies", days: int = 30, count: int = 10):
    ...
    try:
        ...
        logger.exception(f"Failed to get popular content: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/tautulli/history")
async def get_tautulli_history(user: str | None = None, section_id: int | None = None,
                               length: int = 25, days: int | None = None):
